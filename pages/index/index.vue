	...
	<view class="container">
		...
		<scroll-view scroll-top="0" scroll-y="true" class="scroll-Y">
			<uni-list v-for="(item, index) in dataList" :key=index>
				<uni-list-item :class = "index == now ? 'addclass' : '' " :title="item.Name" thumb-size="medium" :thumb="item.AlbumPic" :rightText="item.Singer" clickable  @click="playone(index)"></uni-list-item>
			</uni-list>
	    </scroll-view>
			...
			flush_data(r){
				uni.request({
					...
					success: (res) => {
						...
						this.audio = res.data.map(x => {return x.Audio})
					}
				});
			},
